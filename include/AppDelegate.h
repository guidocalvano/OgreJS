...
- (void)renderOneFrame:(id)sender
{
    ...
    if( /*!OgreManager::getSingletonPtr()->isOgreToBeShutDown() && */
       Ogre::Root::getSingletonPtr() && Ogre::Root::getSingleton().isInitialised())
    {
		if(OgreManager::getSingletonPtr()->m_pRenderWnd->isActive())
		{
			mStartTime = OgreManager::getSingletonPtr()->m_pTimer->getMillisecondsCPU();
            
			OgreManager::getSingletonPtr()->m_pKeyboard->capture();
			OgreManager::getSingletonPtr()->m_pMouse->capture();
            
			// OgreManager::getSingletonPtr()->updateOgre(mLastFrameTime);
			OgreManager::getSingletonPtr()->m_pRoot->renderOneFrame();
            
			mLastFrameTime = OgreManager::getSingletonPtr()->m_pTimer->getMillisecondsCPU() - mStartTime;
		}
    }
    else
    {
